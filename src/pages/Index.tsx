...
function BreathingLungsVisual() {
  return (
    <div
      className="relative mx-auto aspect-square w-full max-w-sm rounded-[2rem] border border-white/80 bg-[radial-gradient(circle_at_50%_35%,hsl(var(--pc-sky))_0%,white_54%,hsl(var(--pc-bg-soft-blue))_100%)] p-6 shadow-2xl shadow-primary/10"
      style={{ maxWidth: "min(24rem, calc(100vw - 4rem))" }}
    >
  ...
  return (
    <PublicLayout>
      ...
      <section className="relative isolate overflow-hidden bg-[#50a5fa]">
        ...
        <div className="absolute inset-x-0 bottom-0 hidden h-28 bg-gradient-to-t from-[#50a5fa] to-transparent md:block" />

        <div className="container relative py-4 pb-2 md:grid md:min-h-[640px] md:grid-cols-[0.92fr_1.08fr] md:items-center md:gap-8 md:py-10 lg:min-h-[700px] lg:gap-12 lg:py-14 xl:min-h-[760px] xl:gap-16">
          <div className="md:hidden">
            ...
            <div
              className="relative z-10 mx-auto mt-4 max-w-full rounded-[2rem] border border-white/80 bg-white p-5 shadow-2xl shadow-primary/10"
            ...
            >
              ...
              <div className="space-y-3">
                <h1 className="font-display text-[2rem] font-bold leading-[0.98] tracking-tight text-foreground min-[430px]:text-[2.35rem]">
            ...
            <div className="mt-4 grid gap-2 text-xs text-muted-foreground">
              {["Online doctors consultation", "Overseas supplier with a variety of treatment options", "Guaranteed prescription fulfilment"].map((item) => (
                <div
                  key={item}
                  className="mx-auto flex max-w-full items-center gap-2 rounded-2xl border border-white/70 bg-white/80 px-3 py-3 shadow-sm backdrop-blur"
                  style={{ width: "calc(100vw - 4rem)" }}
                >
          ...
          <div className="hidden max-w-none space-y-7 rounded-[2.25rem] border border-white/80 bg-white/90 p-7 shadow-2xl shadow-primary/15 backdrop-blur-xl animate-fade-in-up md:block lg:rounded-[2.75rem] lg:p-9 xl:p-11">
            <div className="inline-flex items-center gap-2 rounded-full border border-primary/15 bg-primary/10 px-3 py-2 text-sm font-semibold text-primary shadow-sm backdrop-blur">
              <ShieldCheck className="h-4 w-4" />
              Guaranteed Australian fulfilment with a prescription
            </div>

            <div className="space-y-5">
              <h1 className="font-display text-5xl font-bold leading-[0.98] tracking-tight text-foreground lg:text-[4rem] xl:text-[4.75rem]">
                Alternative Smoking Cessation Care.
              </h1>
              <p className="max-w-2xl text-lg leading-8 text-muted-foreground xl:text-xl xl:leading-9">
                Quit smoking the way you prefer. Discuss your cessation plans with our practitioners and begin your journey for $10 per can.
              </p>
            </div>
            ...
            <div className="grid gap-3 text-sm text-muted-foreground xl:grid-cols-3">
              {[
                ...
                <div key={item} className="flex items-center gap-2 rounded-2xl border border-primary/10 bg-primary/5 px-3 py-3 shadow-sm backdrop-blur">
                  <CheckCircle2 className="h-4 w-4 shrink-0 text-primary" />
                  <span>{item}</span>
                </div>
              ))}
          ...
          <div className="relative hidden w-full animate-fade-in-up animation-delay-200 md:block">
            <div className="absolute -inset-4 rounded-[2.5rem] bg-white/35 blur-2xl" />
            <div className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/95 p-2.5 shadow-2xl shadow-primary/15 md:rounded-[2.5rem] md:p-3 xl:p-4">
              <img
                src="/images/pouchcare-doctor-hero-blue.jpg"
                alt="PouchCare online clinical pathway visual"
                className="aspect-[5/4] w-full rounded-[1.5rem] object-cover object-center md:rounded-[2rem] lg:aspect-[6/5] xl:aspect-[5/4]"
              />
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#50a5fa] py-3 md:bg-[linear-gradient(180deg,#50a5fa_0%,hsl(var(--pc-bg-soft-blue))_100%)] md:py-16 lg:py-20">
        <div className="container flex justify-center md:grid md:grid-cols-[0.8fr_1.2fr] md:items-center md:gap-10 lg:gap-16">
          <div className="md:justify-self-end">
            <BreathingLungsVisual />
          </div>
          <div className="hidden max-w-2xl rounded-[2rem] border border-white/70 bg-white/85 p-8 shadow-xl shadow-primary/10 backdrop-blur md:block lg:p-10">
            <p className="mb-3 text-sm font-bold uppercase tracking-[0.2em] text-primary">Designed for clear next steps</p>
            <h2 className="font-display text-3xl font-bold leading-tight text-foreground lg:text-5xl">A calmer pathway before access is unlocked.</h2>
            <p className="mt-4 text-base leading-8 text-muted-foreground lg:text-lg">
              The pathway is structured to keep eligibility, clinical review, prescription requirements, and fulfilment checks in a clear order before any ordering access is available.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container">
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4 lg:gap-6">
            {processSteps.map((step, index) => (
              <div key={step.title} className="group relative rounded-3xl border border-border bg-card p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl lg:p-7">
      ...
      <section className="overflow-hidden bg-background py-14 md:py-20 lg:py-24">
        <div className="container">
          <div className="mx-auto mb-8 max-w-3xl text-center lg:mb-10">
            <h2 className="font-display text-3xl font-bold text-foreground md:text-5xl lg:text-6xl">Join Thousands of Other Aussies</h2>
          </div>
        </div>

        <div className="relative">
          <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-8 bg-gradient-to-r from-background to-transparent md:w-12 lg:w-28" />
          <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-8 bg-gradient-to-l from-background to-transparent md:w-12 lg:w-28" />
          <div className="flex w-max gap-3 animate-testimonial-marquee md:gap-4 lg:gap-5">
            {[...testimonialVideos, ...testimonialVideos].map((src, index) => (
              <div key={`${src}-${index}`} className="w-28 shrink-0 overflow-hidden rounded-[1.35rem] border border-white/80 bg-white p-1.5 shadow-xl shadow-primary/10 sm:w-32 md:w-36 md:rounded-[1.7rem] lg:w-44 xl:w-52 xl:rounded-[2rem] xl:p-2">
                <video
                  src={src}
